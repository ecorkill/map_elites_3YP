...
def drawRobot(torso, arm, base, goal):
    fig = pt.figure()
    axis = Axes3D(fig)
    axis.set_xlim(0, 2)
    axis.set_ylim(0, 2)
...
def verifyAngles(joints, policy, multiple, jointNum):
    if multiple:
        valid = False
        ...
        if joints[jointNum].getLowerLimit() <= policy <= joints[jointNum].getUpperLimit():
            valid = True
        else:
            valid = False
        # endif
...
def testIndividual(policy, final, goal, verifyAngle, joint):
    ...
    if verifyAngle:
        valid = verifyAngles(joints, policy, False, joint)
        return valid
    else:
        ...
        valid = verifyAngles(joints, policy, True, 0)
        if valid:
            arm = RobotArm(links, joints, policy)
            #if (np.allclose(endPoint, arm.getPosEnd())):
            ...
            if collision:
                ...
                if validTorso:
                    ...
                    if noCollision:
                        # from final set of individuals draw the final outcome
                        if final:
                            drawRobot(torso, arm, base, goal)
                            print("--------------------------------------")
                            return True
                        else:
                            return True, arm.getPosEnd()
                        # endif
                    else:
